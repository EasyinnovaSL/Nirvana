...
use humhub\modules\loginUsers\models\InviteLoginUsers;
use humhub\modules\loginUsers\models\UserInviteGroup;
use humhub\modules\user\models\forms\Login;
use humhub\modules\loginUsers\authclient\AuthClientHelpers;
...
class AuthController extends Controller
{
    ...
    public function actionLogin()
    {
        ...
        /* Check if the user type exist */
        $usertype = Yii::$app->getRequest()->getQueryParam('userType');

        if(!isset($usertype) && ($usertype != INNOVATOR_GROUP_ID || $usertype != INNOVATION_ADVISOR_GROUP_ID) ){
            /* Load error modal */
            if (Yii::$app->request->getIsAjax()) {
                return $this->renderAjax('login_modal_error');
            }else{
                return;
            }
        }
        ...
        // Self Invite
        $invite = new InviteLoginUsers();
        $invite->scenario = 'invite';
        $invite->setGroupType($usertype);
        ...
        if (Yii::$app->request->getIsAjax()) {
            /* Check the userType and load one or onther*/
            if(isset($usertype) && $usertype == INNOVATOR_GROUP_ID){
                return $this->renderAjax('login_modal_innovator', array('model' => $login, 'invite' => $invite, 'canRegister' => $invite->allowSelfInvite()));
            }else if(isset($usertype) && $usertype == INNOVATION_ADVISOR_GROUP_ID){
                return $this->renderAjax('login_modal_innovation_advisors', array('model' => $login, 'invite' => $invite, 'canRegister' => $invite->allowSelfInvite()));
            }else{
                return $this->renderAjax('login_modal_error');
            }
        }
    ...
    public function onAuthSuccess(\yii\authclient\BaseClient $authClient)
    {
       ...
       if(isset($authClient) && $authClient->getName() == LINKEDIN_VALUE){

           /* Get paremeters from URL to determine the type */
           $returnUrl = $authClient->__get("returnUrl");
           $parts = parse_url($returnUrl);
           parse_str($parts['query'], $query);
           //$userTypeUrl = $query['userType'];

           if(isset($query['userType'])){
               $userTypeUrl = $query['userType'];
           }else if(isset($query['token'])){
               $socialNetworkTokenValue = $query['token'];

               $linkedInEmail = $attributes['email'];

               /* Search the user in the database */
               $userInviteObject = InviteLoginUsers::findOne(['token' => $socialNetworkTokenValue]);

               /* First find the old email */
               $existingUserInviteGroup = UserInviteGroup::findOne(['user_invite_id' => $userInviteObject->id]);

               $userTypeUrl = $existingUserInviteGroup->group_id;

               /* Update the user invitation table with the likedin email */
               $userInviteObject->email = $linkedInEmail;
               $resultUpdateInviteUser = $userInviteObject->save();

               /* Remove row in user_invite*/
           }

           $session = Yii::$app->session;
           $session->set('linkedinUserType', $userTypeUrl);

           $listUserAttributes = $authClient->getUserAttributes();
           ArrayHelper::remove($listUserAttributes, 'group_id');
           $listAtributesAuthUser = ArrayHelper::merge(['group_id' => $userTypeUrl], $listUserAttributes);
           $authClient->setUserAttributes($listAtributesAuthUser);
        }

        // Try automatically create user & login user (for example if it comes on linkedin)
        ...
        return $this->redirect(['/loginUsers/registration']);
    }
