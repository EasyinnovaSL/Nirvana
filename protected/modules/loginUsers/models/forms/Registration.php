...
namespace humhub\modules\loginUsers\models\forms;
...
class Registration extends HForm
{
    ...
    public function register(\yii\authclient\ClientInterface $authClient = null)
    {
        ...
        /* We need to delete the field on user_invite_group to avoid constrain violation */
        $userInvite = Invite::findOne(['email' => $this->models['User']->email]);
        $userInviteId = $userInvite->id;
        $existingUserInviteGroup = UserInviteGroup::findOne(['user_invite_id' => $userInviteId]);

        /* Create a transaction and delete user_invite_group and user_invite rows*/
    ...
    public function setGroupUserId($groupId = null){
        if(isset($this->_groupUser)){
            $this->_groupUser->group_id = $groupId;
        }
    }

    /**
     * Returns the value of the id of the user's group
     *
     * @return group_id
     */
    public function getGroupUserId(){
        if(isset($this->_groupUser)){
            return $this->_groupUser->group_id;
        }else{
            return null;
        }
    }
