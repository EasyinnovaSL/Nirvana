    ...
    public function actionCreate($visibility = null)
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
           // return $this->actionModules($model->id);
            return $this->actionInvite($model);
        }
        ...
        $types = [];
        foreach (Type::find()->orderBy(['sort_key' => SORT_ASC])->all() as $type) {
        	$types[$type->id] = $type->title;
        }

        return $this->renderAjax('create', ['model' => $model, 'visibilityOptions' => $visibilityOptions, 'joinPolicyOptions' => $joinPolicyOptions, 'types' => $types]);
    }
