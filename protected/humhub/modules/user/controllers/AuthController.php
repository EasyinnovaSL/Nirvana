...
class AuthController extends Controller
{
    ...
    public function onAuthSuccess(\yii\authclient\BaseClient $authClient)
    {
       ...
       /* if(isset($authClient) && $authClient->getName() == LINKEDIN_VALUE){
            ...
            $emailAdress = ArrayHelper::getValue($listUserAttributes, 'email-address');
            $firstName = ArrayHelper::getValue($listUserAttributes, 'first-name');
            $lastName = ArrayHelper::getValue($listUserAttributes, 'last-name');

            ArrayHelper::remove($listUserAttributes, 'email-address');
            ArrayHelper::remove($listUserAttributes, 'first-name');
            ArrayHelper::remove($listUserAttributes, 'last-name');

            $listAtributesAuthUser = ArrayHelper::merge(['group_id' => INNOVATOR_GROUP_ID, 'username' => $firstName.' '.$lastName, 'name'=> $firstName.' '.$lastName,'firstname' => $firstName, 'lastname' => $lastName,'email' => $emailAdress], $listUserAttributes);
